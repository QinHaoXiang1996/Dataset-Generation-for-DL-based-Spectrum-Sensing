#!/usr/bin/env python
# This is the main program for dataser generation
...
import random
dataset = {}
nvecs_per_key = 10000 # nvecs_per_key is the number of signal samples which represent hypothesis H1
vec_length = 128 # vec_length is sample length
...
for snr in snr_vals:
    ...
    for i,mod_type in enumerate(transmitters[alphabet_type]):
        ...
        # if insufficient_modsnr_vectors is "true", we use GNU radio to get a discrete time sequence
        modvec_indx = 0# modvec_indx is the total number of samples already collected
        ...
        # we use the simulation method in "T. O’Shea and N. West, “Radio machine learning dataset generation with gnu radio,” in Proc. GNU Radio Conference (GRCon16), vol. 1, 2016."
        while insufficient_modsnr_vectors:
            ...
            while sampler_indx + vec_length < len(raw_output_vector) and modvec_indx < nvecs_per_key:
                ...
                npower=xpower/snr_x
                random_noise = (np.random.randn(vec_length)+1j*np.random.randn(vec_length))*np.sqrt(npower) # complex AWGN at a specific noise power level corresponding to the desired SNR.
                ...
                dataset[(mod_type.modname, snr)][modvec_indx,:,0] = np.real(total_vector) # now we package data
